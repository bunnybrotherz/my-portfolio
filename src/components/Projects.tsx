...
const projects = [{
  ...
  highlights: ["30+ user research participants", "4 design iterations based on feedback", "10+ usability testing sessions", "Full design-to-dev ownership"],
  ...
  highlights: ["Lo-fi to Hi-fi design lifecycle", "RESTful API architecture", "Cloud Run deployment", "Banking UX best practices"],
  ...
  highlights: ["Agent-based simulation", "88% rescue rate achieved", "Multilingual interface", "Mobile-first UX design"],
...
const Projects = () => {
  return <section id="projects" className="py-8 sm:py-10 px-4 sm:px-6 relative">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {projects.map((project, index) => <motion.article key={index} initial={{
          opacity: 0,
          x: index % 2 === 0 ? -40 : 40,
          scale: 0.95
        }} whileInView={{
          ...
          ease: [0.25, 0.46, 0.45, 0.94]
              ...
              <Link to={`/projects/${project.slug}`} className="block h-full">
                <motion.div className={`absolute inset-0 bg-gradient-to-r ${project.gradient} rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500`} initial={{
              scale: 0.8,
              opacity: 0
            }} whileInView={{
              scale: 1,
              ...
              delay: index * 0.15 + 0.2
                ...
                <div className="flex flex-col gap-4 sm:gap-6 flex-1">
                  {/* Project Images */}
                  <motion.div className="flex-shrink-0 w-full" initial={{
                  opacity: 0,
                  scale: 0.9
                }} whileInView={{
                  opacity: 1,
                  ...
                  delay: index * 0.15 + 0.2
                    ...
                    {project.mainImage ? <div className="space-y-3 w-full">
                        {/* Main image - with or without laptop mockup */}
                        {project.useLaptopMockup ? <div className="relative w-full">
                            <div className="bg-[#2d2d2d] rounded-t-lg pt-2 pb-1 px-2">
                              ...
                              <img src={project.mainImage} alt={`${project.title} main screenshot`} className="w-full h-auto object-contain rounded-sm" />
                            </div>
                            <div className="bg-[#1a1a1a] h-2.5 rounded-b-lg" />
                            <div className="bg-[#2d2d2d] h-1 w-1/3 mx-auto rounded-b-md" />
                          </div> : <img src={project.mainImage} alt={`${project.title} main screenshot`} className="w-full h-64 object-cover rounded-lg shadow-md group-hover:scale-[1.02] transition-transform duration-500" />}
                      ...
                      </div> : project.images ? <div className="grid grid-cols-4 gap-2 w-full h-64 overflow-hidden">
                        {project.images.map((img, i) => <img key={i} src={img} alt={`${project.title} screenshot ${i + 1}`} className="w-full h-64 object-cover object-top rounded-md group-hover:scale-105 transition-transform duration-500" />)}
                      </div> : <div className="h-64 w-full rounded-lg overflow-hidden bg-secondary/50 border border-border flex flex-col items-center justify-center text-muted-foreground/50">
                  ...
                  <div className="flex-1 min-w-0 relative flex flex-col">
                    ...
                    <ArrowUpRight size={24} className="absolute top-0 right-0 text-muted-foreground group-hover:text-primary group-hover:translate-x-1 group-hover:-translate-y-1 transition-all" />
                    
                    <motion.div className="pr-10 mb-4" initial={{
                    opacity: 0,
                    y: 15
                  }} whileInView={{
                    opacity: 1,
                    ...
                    delay: index * 0.15 + 0.2
                      ...
                      <div className="text-muted-foreground text-sm sm:text-base">
                        ...
                        {project.partner && <span className="inline-flex items-center gap-1.5 text-primary font-medium ml-2">
                    ...
                    <motion.p className="text-sm sm:text-base text-muted-foreground mb-4 sm:mb-6 max-w-2xl" initial={{
                    opacity: 0
                  }} whileInView={{
                    opacity: 1
                  }} viewport={{
                    ...
                    <motion.div className="flex flex-wrap gap-2 sm:gap-4 text-xs sm:text-sm text-muted-foreground mb-4" initial={{
                    opacity: 0
                  }} whileInView={{
                    opacity: 1
                  }} viewport={{
                    ...
                    delay: index * 0.15 + 0.4
                      ...
                      {project.highlights.map((highlight, i) => <span key={i} className="flex items-center gap-1.5">
                        ...
                        delay: index * 0.15 + 0.5
                          ...
                          return <Tooltip key={i}>
                                <TooltipTrigger asChild>
                                  <motion.div className="w-8 h-8 flex items-center justify-center cursor-pointer" initial={{
                                opacity: 0,
                                scale: 0.8
                              }} whileInView={{
                                opacity: 1,
                                ...
                                scale: 1.15
                                  ...
                                  {tool === "AI" ? <Sparkles size={24} className="text-purple-400" /> : <img src={iconUrl} alt={tool} className={`object-contain rounded-md ${tool === "Google Cloud Platform" ? "w-16 h-16" : "w-8 h-8"}`} />}
                                  </motion.div>
                                </TooltipTrigger>
                                <TooltipContent>
                                  <p>{tool}</p>
