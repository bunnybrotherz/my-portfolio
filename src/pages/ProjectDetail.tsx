...
const projectsData: Record<string, {
  ...
  "getgoing": {
    ...
    highlights: ["Agent-based simulation model", "88% rescue rate achieved", "Multilingual mobile interface", "5 prioritization strategies tested"],
        ...
        <div className="max-w-4xl mx-auto">
          ...
          duration: 0.5
            ...
            <div className="flex flex-wrap items-center gap-3 text-muted-foreground">
              ...
              {project.partner && <span className="inline-flex items-center gap-1.5 text-primary font-medium">· In collaboration with Gebirah
                </span>}
          ...
          {/* Main Image */}
          {project.mainImage && <motion.div initial={{
          opacity: 0,
          scale: 0.95
        }} animate={{
          opacity: 1,
          ...
          delay: 0.1
              ...
              <img src={project.mainImage} alt={`${project.title} main screenshot`} className="w-full h-auto rounded-xl shadow-lg" />
            </motion.div>}
          ...
          delay: 0.3
            ...
            <div className="grid grid-cols-2 gap-3">
              {project.highlights.map((highlight, i) => <div key={i} className="flex items-center gap-2 text-muted-foreground">
              ...
              <div className="flex flex-wrap gap-4">
                ...
                return <Tooltip key={i}>
                      <TooltipTrigger asChild>
                        <motion.div className="w-10 h-10 flex items-center justify-center cursor-pointer" whileHover={{
                      scale: 1.15
                    }}>
                          {tool === "AI" ? <Sparkles size={28} className="text-purple-400" /> : <img src={iconUrl} alt={tool} className={`object-contain rounded-md ${tool === "Google Cloud Platform" ? "w-20 h-20" : "w-10 h-10"}`} />}
                        </motion.div>
                      </TooltipTrigger>
                      <TooltipContent>
                        <p>{tool}</p>
          ...
          {/* Images Gallery */}
          {project.images.length > 0 && <motion.div initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
          ...
          delay: 0.4
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {project.images.map((img, i) => <motion.img key={i} src={img} alt={`${project.title} screenshot ${i + 1}`} className="w-full h-auto rounded-lg shadow-md hover:scale-105 transition-transform duration-300" initial={{
              opacity: 0,
              scale: 0.9
            }} animate={{
              opacity: 1,
